...
function finder(treasure) {
  if (!treasure.length || treasure.length === 1) return 'Harta karun tidak ditemukan'
  let result = 0
  ...
  for (let i = 0; i < treasure.length; i++) {
    for (let j = 0; j < treasure[i].length; j++) {
      if (
        ...
        result++
      } else if (Number(treasure[i][j])) {
        console.log(treasure[i][j])
        result -= Number(treasure[i][j])
      }
    }
  }
  result
  if (result) {
    return `Berhasil memperoleh ${result} harta karun`
  } else {
...
console.log(
  finder(['kjsadf', 'hlkjhasfd', 'lxcmvnbzcv', 'baksljdslkjdfh', 'awiuekjsadf', 'jkagfkjg99'])
